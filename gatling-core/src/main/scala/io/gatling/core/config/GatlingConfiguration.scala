...
object GatlingConfiguration extends StrictLogging {
  ...
  自动停止压测的指标选项
   */
  private def indexConfiguration(config: Config) =
    new Index(
      activeUsers = config.getLong(index.ActiveUsers),
      responseTimePercentiles3 = config.getLong(index.ResponseTimePercentiles3),
      requestKoPercentage = config.getLong(index.RequestKoPercentage),
      responseTimePercentage = config.getLong(index.ResponseTimePercentage)
    )
  ...
  private def mapToGatlingConfig(config: Config) =
    new GatlingConfiguration(
      ...
      index = indexConfiguration(config)
      // [fl]
      //
      // [fl]
    )
...
final class Index(
    ...
    val responseTimePercentiles3: Long,
    val requestKoPercentage: Long,
    val responseTimePercentage: Long
)
